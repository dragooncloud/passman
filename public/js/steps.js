jQuery(document).ready(function($){
	...
	// maintain context object
	var context = { user: null };
	document.addEventListener('context-update', (e) => {
		var keys = Object.keys(e.detail);
		for (var i = 0; i < keys.length; i++) {
			console.debug('updating context with property', keys[i]);
			context[keys[i]] = e.detail[keys[i]];
		}
	});

	// custom event allows us to subscribe loosly
	const fireEventStepped = (stepName) => document.dispatchEvent(
		new CustomEvent('stepped', {
		detail: { stepName, context },
	}));
	fireEventStepped('start'); // fire for first step	
	...
	//start animation
	function triggerAnimation(newSection) {
		console.log('animation', newSection);
		isAnimating =  true;
		newSection = ( newSection == '' ) ? 'start' : newSection;
		...
		//load new content
		return loadNewContent(newSection);
	}
	...
	function loadNewContent(newSection) {
		return new Promise(function (resolve, reject) {
			setTimeout(() => {
				...
				var fileToLoad = newSection === 'start' ? 'start' : 'partials/_' + newSection;
				//load the new content from the proper html file
				section.load(fileToLoad+'.html .cd-section > *', function(event){
					...
					loadingBar.velocity('stop').velocity({
						...
						section.prev('.visible').removeClass('visible').end().addClass('visible')
							...
							function(){
								...
								console.log('next step loaded', newSection);
								fireEventStepped(newSection);
	...
	window.proceedToStep = triggerAnimation;
});